...
// Art of Balance gets its own ranking tables (artofbalance_rankings etc.),
...
func ArtOfBalanceGetRankingsAndCountByCategoryAndRankingOrderParam(category types.UInt32, rankingOrderParam ranking_types.RankingOrderParam) (types.List[ranking_types.RankingRankData], uint32, error) {
	return ArtOfBalanceGetRankings(
...
// Same ranking modes as the other servers in this project: range(0), near(1)
// and friend-range(2). No friends system yet, so friend-range degrades to
// the user-mode query (the protocol's own documented fallback).
const rankingModeNearArtOfBalance = 1
const rankingModeFriendRangeArtOfBalance = 2

func ArtOfBalanceGetRankings(
	...
	if mode == rankingModeFriendRangeArtOfBalance {
		mode = rankingModeUser
	}
	if mode != rankingModeRange && mode != rankingModeUser && mode != rankingModeNearArtOfBalance {
	...
	if mode == rankingModeUser || mode == rankingModeNearArtOfBalance {
		offset = 0
	}
	...
	rows, err := Postgres.Query(`
		WITH base AS (
			...
			FROM artofbalance_rankings AS r
			LEFT JOIN artofbalance_ranking_categories AS rc
				ON rc.category = r.category
			LEFT JOIN artofbalance_common_datas AS cd
