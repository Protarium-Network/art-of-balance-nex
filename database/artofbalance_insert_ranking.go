...
func ArtOfBalanceInsertRankingByPIDAndRankingScoreData(pid types.PID, rankingScoreData ranking_types.RankingScoreData, uniqueID types.UInt64) error {
	...
	_, err = tx.Exec(`
		INSERT INTO artofbalance_ranking_categories (category, order_by, created_at)
		VALUES ($1, $2, $3)
	...
	result, err := tx.Exec(`
		INSERT INTO artofbalance_rankings (
		...
		WHERE
			...
			OR (artofbalance_rankings.order_by = 0 AND EXCLUDED.score < artofbalance_rankings.score)
			OR (artofbalance_rankings.order_by = 1 AND EXCLUDED.score > artofbalance_rankings.score)
	`,
		int64(pid),
		int64(uniqueID),
		int64(rankingScoreData.Category),
	...
	globals.Logger.Infof("[ARTOFBALANCE RANKING] upload PID=%d category=%d score=%d stored=%t",
